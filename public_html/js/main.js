...
$(window).scroll(function () {
    var vw = $(document).width() * 0.03;
    var hT = $('.servicesSection').offset().top,
            hH = $('.servicesSection').outerHeight(),
            wH = $(window).height(),
            wS = $(this).scrollTop();
    if (wS + vw > (hT + hH - wH) && scrollState === false) {
        $('#menuButton').css('filter', 'invert(100%)');
        $('.fixedHeadline').css({opacity: 1});

        scrollState = true;
    }

    if (wS + vw < (hT + hH - wH) && scrollState === true) {
        $('#menuButton').css('filter', 'invert(0%)');
        $('.fixedHeadline').css({opacity: 0});
        scrollState = false;
    }
